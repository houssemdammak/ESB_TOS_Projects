...
package web_services_projects.ws_test_0_1;
 ...
 * @status
 */
public class ws_test implements TalendJob {
	...
	private final String jobName = "ws_test";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(ws_test.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class RestServiceProviderImpl4TalendJob {
		...
		private final ws_test job;

		public RestServiceProviderImpl4TalendJob(ws_test job) {
			this.job = job;
		}

		private void populateRequestWithJobContext(java.util.Map<String, Object> requestGlobalMap, ws_test job) {
	...
	class Thread4RestServiceProviderEndpoint extends Thread {
		...
		public Thread4RestServiceProviderEndpoint(ws_test job, String endpointUrl) {
			this.job = job;
	...
	public static class ContextBean {
		static String evaluate(String context, String contextExpression)
			...
			java.io.InputStream inContext = ws_test.class.getClassLoader()
					.getResourceAsStream("web_services_projects/ws_test_0_1/contexts/" + context + ".properties");
			if (inContext == null) {
				inContext = ws_test.class.getClassLoader()
	...
	public static class outStruct implements routines.system.IPersistableRow<outStruct> {
		final static byte[] commonByteArrayLock_WEB_SERVICES_PROJECTS_ws_test = new byte[0];
		static byte[] commonByteArray_WEB_SERVICES_PROJECTS_ws_test = new byte[0];
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_WEB_SERVICES_PROJECTS_ws_test) {
	...
	public static class helloStruct implements routines.system.IPersistableRow<helloStruct> {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_WEB_SERVICES_PROJECTS_ws_test.length) {
					if (length < 1024 && commonByteArray_WEB_SERVICES_PROJECTS_ws_test.length == 0) {
						commonByteArray_WEB_SERVICES_PROJECTS_ws_test = new byte[1024];
					} else {
						commonByteArray_WEB_SERVICES_PROJECTS_ws_test = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_WEB_SERVICES_PROJECTS_ws_test, 0, length);
				strReturn = new String(commonByteArray_WEB_SERVICES_PROJECTS_ws_test, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_WEB_SERVICES_PROJECTS_ws_test, 0, length);
				strReturn = new String(commonByteArray_WEB_SERVICES_PROJECTS_ws_test, 0, length, utf8Charset);
			}
			return strReturn;
		}
...
// Start of branch "out"
	...
	public static void main(String[] args) {
		final ws_test ws_testClass = new ws_test();

		int exitCode = ws_testClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = ws_test.class.getClassLoader()
					.getResourceAsStream("web_services_projects/ws_test_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = ws_test.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : ws_test");
		}
...
/************************************************************************************************
 * 78309 characters generated by Talend Open Studio for ESB on the 4 avril 2025
 * à 10:24:04 WAT
 ************************************************************************************************/